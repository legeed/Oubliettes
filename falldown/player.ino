...
boolean playerCollision(){ //détecte si collision avec un mur ou une plateforme
  for(byte i=0; i<NUM_PLATEFORMS; i++){
    if(gb.collideRectRect((int)player.x, (int)player.y, player.w, player.h, plateforms[i].x, plateforms[i].y, plateforms[i].w, plateforms[i].h)){
      return true;
    }
  }
  for(byte i=0; i<NUM_BORDERS; i++){
    if(gb.collideRectRect((int)player.x, (int)player.y, player.w, player.h, borders[i].x, borders[i].y, borders[i].w, borders[i].h)){
      return true;
    }
  }
  return false;
...
boolean playerLanding(){ //détecte si le joueur est SUR une plateforme
  for(byte i=0; i<NUM_PLATEFORMS; i++){
    if(gb.collideRectRect((int)player.x, (int)player.y, player.w, player.h, plateforms[i].x+1, plateforms[i].y-tilesize, plateforms[i].w-2, plateforms[i].h)){
      return true;
    }
    return false;
  }
